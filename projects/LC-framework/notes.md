...
`python3 utils/moatlib.py audit-commits LC-framework` reports the following. They are recorded
rather than fixed: fixing a commit message rewrites its sha, and the branch carries
validated platforms.

| commit | offence | effect if published |
|---|---|---|
| `ea0df9b` | Test Plan uses an indented block rather than a fenced one | none in practice: the block renders as code on GitHub. AGENTS.md asks for fenced blocks specifically, so the gate flags it. This is the whole of the offence list for this branch |
