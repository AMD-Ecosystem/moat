...
There is no lead platform. Any arch may start the work; a `{arch, since}` porting lock in status.json covers writes to the fork only, and validation runs unlocked and in parallel. Takeover of a held lock is a human decision, not a timeout.
...
Requires explicit approval (show the draft, wait for yes): any GitHub-visible action against an UPSTREAM repo -- a PR or issue comment, a review, an edited PR body -- or anything else visible outside our own forks. Each post is its own act and needs its own yes; an approval never carries to the next one.
...
Windows host (gfx1201): this is a workstation with a single discrete AMD GPU, a gfx1201 RX 9070 XT. The gfx1101 Radeon PRO V710 it formerly carried is gone (which is why gfx1101 is an optional target); with one GPU the card is likely HIP device 0, so older notes or scripts that pin `HIP_VISIBLE_DEVICES=1` are stale -- verify with `hipInfo`/`rocminfo` rather than assuming an index. It is NOT power-constrained -- use normal build parallelism. Build/run via the TheRock PyTorch venv -- a full ROCm distribution -- and NOT the Windows HIP SDK, whose narrower runtime makes a correct port fail at launch in ways that look like port bugs (see the skill's `references/validation.md`). Historical, only relevant if a second GPU is ever installed: with two GPUs visible in one process the ROCm 7.14 HIP runtime crashed (heterogeneous RDNA3+RDNA4 multi-GPU bug), so pin `HIP_VISIBLE_DEVICES` to exactly one GPU per process. The retired gfx1151 Strix Halo APU host had a shared CPU/iGPU power rail and needed `-j6` caps; that constraint does NOT apply here.
