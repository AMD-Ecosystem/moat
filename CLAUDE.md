...
A failure is evidence about ONE COMMIT, exactly as a validation is. `validation-failed` records the `failed_sha` it happened at, and the arch is the porter's only while that sha is still the head -- the porter's fix advances head_sha, and from that moment the arch is owed a validation instead, with no sweep and nothing rewriting the block. That is what closes the loop: while the failure was a bare word, the same arch was dispatched to the porter again after every fix, forever. A delta that cannot change compiled output (a documentation commit) carries the FAILURE forward rather than retiring it -- it cannot have been the fix.
...
**A maintainer may tell us to stop, and recording that is the ONE decision of this kind an agent may make on its own** (`python3 utils/optout.py record <owner|owner/repo> --source <where they asked>`). Everything else above needs a person because it is our judgement; this is somebody else's decision about us, carried into the record, and the only thing it can do is less work. It binds mechanically -- discovery drops those repos, `scaffold` refuses them, `pr_ready` refuses them, so both routes upstream are closed -- and it retires the adopted projects it covers as `opted-out` dispositions. It applies to work already finished, not just work not yet started: an opt-out usually arrives BECAUSE a PR showed up. Closing that PR and deleting the fork are visible acts on their repository and stay with a person; `optout.py record` prints the commands. Removing an opt-out needs a person and `--by`. Never argue with one or ask for a reason.

Every PR body carries a standing note (`SUBMISSION_NOTE` in `utils/upstream.py`): AI-prepared, person-approved, a link back to this repo, and how to opt out. `upstream.py --review` appends it, so do NOT write it by hand -- what is approved is what is published, and the publish gate refuses a body that lost it.
...
- No MOAT vocabulary in upstream-visible text (commit messages, code comments, PR titles/bodies). Keep the technical rationale, drop the in-house labels -- this text goes to external maintainers. The terms and their replacements live in `config/jargon.toml`; check with `python3 utils/jargon.py --port <name>` before pushing, and add any term the checker missed in the same change that found it. It scans the WHOLE branch, never just the commits you added: a message that passed an earlier round is still in the branch and still ships with it. The review-PR gate runs the same scan, so this is verified rather than remembered.
...
- findings/<slug>/ -- prepared ROCm-component bug reports and reproducers. Gitignored, so it is local to a working copy and not published: an unfiled report in a porting repo is one the component owners will never see. The deferral registry below is the durable record of what was found and whether it was filed; file the report properly rather than leaving it here.
- The deferred-work registry: what we postponed and where to resume it. **A deferral belongs to the project that deferred it** -- `projects/<name>/deferred.json`, on that project's branch, next to the notes and the plan it came out of, so it is reviewed with the port that produced it rather than broadcast on its own. `data/deferred.json` is only for what is genuinely NOT project-scoped: a ROCm-component bug with no port attached, and work deferred by a project since removed. **`utils/deferred.py add` writes the GLOBAL file unless you pass `--project <name>`** -- pass it, or the deferral lands in the wrong store, which is why a third of the global file's items still name a project. Kinds: rocm-bug-report, feature-port, other. Ask MOAT "what did we defer?" with `python3 utils/deferred.py list`, which reads both and across refs; `deferred.py pending` is the subset no person has ruled on, and a ruling (`decide <id> --choice defer|now --by <who>`) is a person's, never an agent's. When you scope a sub-feature out of a port or prepare a findings/ bug report you do not file, register it so it is not lost.
