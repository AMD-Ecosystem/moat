...
- `codeobj_diff.py`'s device-ISA comparison needs `roc-obj-ls` and hardcodes `/opt/rocm` for it, plus `llvm-nm`/`llvm-objdump`. A host whose ROCm is the pip SDK (packages under a conda env's `_rocm_sdk_devel`, the layout `notes.md` build recipes already point `clang++` at) has no `/opt/rocm` tree, and in at least one observed SDK build `roc-obj-ls`'s python entry point (`rocm_sdk_core._cli.roc_obj_ls`) does not exist and there is no standalone script substitute -- only `clang-offload-bundler`, a different interface. On such a host the binary-equivalence carry-forward path is simply unavailable; per the tool's own contract ("if you cannot build both shas, do the normal full real-GPU revalidation") that is not a blocker, just a fallback to the ordinary GPU run. Do not spend time reimplementing `roc-obj-ls`; note the gap and revalidate for real.
...
- PR-prep gate -- nvcc CUDA-build check: before opening any PR whose claim is "the CUDA/NVIDIA build is unchanged", compile the CUDA path (`USE_HIP=OFF`) with nvcc on this GPU-less host (nvcc from the dedicated conda env `/opt/conda/envs/cuda-12.8/bin/nvcc`, created if missing with `conda create -y -n cuda-12.8 -c nvidia cuda-toolkit=12.8` -- the full `cuda-toolkit`, not compiler-only `cuda-nvcc`, because the check must LINK; host gcc 13 works. ALWAYS pin the arch, e.g. `-DCMAKE_CUDA_ARCHITECTURES=80`: `native` autodetection on a host with no NVIDIA GPU silently degrades to an ancient arch, and `atomicAdd(double*)` "no instance of overloaded function" is that fingerprint, not a real failure). nvcc compiles without an NVIDIA GPU, so this is available on a ROCm-only host, and GPU validation on AMD cannot see a broken CUDA path -- "notes.md says the CUDA path is preserved" is NOT the same as having compiled it. State it honestly in the PR ("compile-checked with nvcc, not run"). REQUIRED when the port adds a CUDA/HIP-split backend (e.g. `Foo_cuda.cpp` vs `Foo_hip.cpp`): that CUDA file is never compiled during HIP-only porting and can be badly broken. The check must reach the LINK stage (build one real target/demo, not compile-only of a single TU) -- the undefined-reference class (e.g. explicit instantiations that do not match real call-site signatures) only surfaces at link. If the fix is HIP-binary-equivalent (codeobj_diff identical), it carries forward with no GPU re-validation.
  ...
  - A rapids-cmake `rapids_hip_init_architectures` HIP-dependency (e.g. hipMM/RMM) can fail configure on a multi-GPU host with `mismatch between CMAKE_HIP_ARCHITECTURES and GPU_TARGETS` even though only one arch was requested: ROCm's `hip-config.cmake` auto-populates the `GPU_TARGETS` cache variable by enumerating every visible device rather than the set of distinct architectures, so four cards of the same gfx report `GPU_TARGETS='gfx1100;gfx1100;gfx1100;gfx1100'` against a single-valued `CMAKE_HIP_ARCHITECTURES='gfx1100'`, and the rapids-cmake string-equality check (not a set comparison) trips. This is a host-population artifact, not a port defect -- confirm by rerunning the identical configure with `HIP_VISIBLE_DEVICES=0` (or on a single-GPU host) and observing it pass. Work around it at the configure invocation, no source edit needed: pass `-DGPU_TARGETS=<arch> -DAMDGPU_TARGETS=<arch>` alongside `-DCMAKE_HIP_ARCHITECTURES=<arch>` so all three agree before rapids-cmake's check ever runs. (HEonGPU, gfx1100 4-GPU host)
