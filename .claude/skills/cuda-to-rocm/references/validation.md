...
- `codeobj_diff` pointed at a whole build directory can return `verdict=indeterminate` even when the project's own binary is genuinely identical: CMake writes its own compiler-detection throwaways into every configure (`CMakeFiles/<ver>/CMakeDetermineCompilerABI_*.bin`, `CMakeFiles/<ver>/CompilerId*/a.out`), and those carry no device code, so extraction on them fails and drags the overall verdict down regardless of the real target. This is tool noise, not ambiguity about the port: point the tool directly at the actual build product(s) instead of the build directory (`codeobj_diff <old>/build/<exe> <new>/build/<exe>`) and read the verdict on that. Seen on GPU_IPC (`gipc`): whole-dir verdict `indeterminate` (4 CMake-internal entries), direct-binary verdict `identical`.
