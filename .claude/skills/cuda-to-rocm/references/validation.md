...
- `utils/codeobj_diff.py` hardcodes `/opt/rocm/bin/roc-obj-ls` and `/opt/rocm/llvm/bin/llvm-nm`. On a host whose ROCm is only the `_rocm_sdk_devel`/`_rocm_sdk_core`/`_rocm_sdk_libraries` conda packages (TheRock's pip/conda distribution) there is no `/opt/rocm` at all, and `roc-obj-ls` is not shipped anywhere in those packages (confirmed by `find` over all three) -- the script fails with `FileNotFoundError`, not a wrong verdict, so the failure is easy to notice. On such a host fall back to the manual same-directory pair the tool's own docstring describes: rebuild both shas IN ONE DIRECTORY (`git checkout <sha> -- <file>`, build, capture, `git checkout <other sha> -- <file>`, rebuild in place), dump `.hip_fatbin` with `<sdk>/lib/llvm/bin/llvm-objcopy --dump-section=.hip_fatbin` and sha256-compare, plus `<sdk>/lib/llvm/bin/llvm-nm -D --defined-only <exe>` sorted-diffed across the two builds for exported symbols. That combination is sound (same-directory rebuild sidesteps the `__hip_cuid_`/build-path artifact) even without the ISA-normalizing cross-directory check `codeobj_diff.py` would otherwise provide (TurboFNO linux-gfx90a, ROCm 7.14 TheRock nightly, 2026-08-24).
