...
## Windows clang-offload-bundler `-fgpu-rdc` "user-mapped section open" -- re-check before assuming still-blocked

A `-fgpu-rdc` build under clang-cl on Windows can fail with `clang-offload-bundler: error: '...': The requested operation cannot be performed on a file with a user-mapped section open` (ROCm/TheRock#5615). Root cause: clang-cl's `/Fo<path>` compiles the host pass to `<path>` and then hands `clang-offload-bundler` that SAME path as both `-input` and `-output`; Windows refuses to rewrite a file with an open memory mapping (`ERROR_USER_MAPPED_FILE`, 0x4C8). It is the `/Fo` vs `-o` distinction, not object size or parallelism.

Two independent fixes, either sufficient alone: (a) override the HIP compile rule to emit `-o <path>` instead of `/Fo<path>` so the host object routes to a distinct temp file (input != output); (b) pass `--offload-new-driver` on both the compile and link steps, which bypasses the same bundler code path. If a project's CMake already carries fix (a) (grep for `CMAKE_HIP_COMPILE_OBJECT` overrides and a comment referencing this bug or `TheRock#5615`), do not assume the project is still blocked just because an OLDER validation record says so -- re-run the build first. A newer TheRock SDK can also simply stop hitting the bug even without either fix; treat a stale BLOCKED record from a prior SDK version as a hypothesis to re-test, not a standing fact. (alien, windows-gfx1151: a 2026-06 BLOCKED record was fully cleared by the porter's own fix (a) landing on the branch; a later re-validation on a newer SDK found it already fixed and never needed to try (b).)

## Windows LIB/INCLUDE env vars: git-bash POSIX paths are not auto-translated

`PATH` gets MSYS/git-bash's automatic POSIX-to-Windows path translation when a real Windows `.exe` is spawned, but arbitrary env vars like `LIB` and `INCLUDE` do NOT. Setting `export LIB="/c/Program Files (x86)/.../lib/x64;..."` from bash passes that literal string through to `lld-link.exe`, which cannot resolve a `/c/...` path -- it fails with `lld-link: error: could not open 'kernel32.lib': no such file or directory` even though the file is right there, because the whole LIB list silently failed to resolve. The fix is not quoting; it is using native Windows-style paths (`C:/Program Files (x86)/...` -- forward slashes are fine, the drive-letter form is what matters) for LIB, INCLUDE, LIBPATH, and any other MSVC-toolchain-consumed path list, e.g. via `cygpath -w`.

## CMake 3.31 can fail HIP-language ABI detection under clang-cl; a newer CMake fixes it

`enable_language(HIP)` with `CMAKE_HIP_COMPILER` set to clang-cl can fail its own ABI-detection try_compile with `CMake Error ... Windows-MSVC.cmake:69 (message): MSVC compiler version not detected properly:` on CMake 3.31 -- this happens inside CMake's own scratch project, before any of the port's CMakeLists code runs, so it is not a port defect. A newer CMake (4.x, `pip install --user cmake` is the fastest way to get one on a locked-down Windows host) has been observed to clear it with no other change. If a Windows/HIP configure dies at "Detecting HIP compiler ABI info" with that exact message, try a newer CMake before treating it as a port or toolchain block.

## Mixing clang-cl compiler ID with a GCC-driver link command corrupts response files

A project that detects `CMAKE_HIP_COMPILER_FRONTEND_VARIANT STREQUAL "MSVC"` (clang-cl) but then routes the actual HIP device-link through the GCC-driver `clang.exe` (needed because clang-cl's link mode cannot do `-fgpu-rdc --hip-link`) inherits a mismatch: CMake writes the linker's `@file.rsp` in MSVC/Windows quoting (raw literal backslashes, since it believes the consumer is an MSVC-family tool) but `clang.exe`'s GNU-style `@file` parser treats backslash as a shell escape character and strips every one, corrupting every path in the response file (`CMakeFiles\Foo.dir\bar.cpp.obj` becomes `CMakeFilesFoo.dirbar.cpp.obj`, and the link fails with a wall of "no such file or directory" for names that never had a chance to resolve). This only shows up once a target's object-file list crosses the length threshold where CMake/ninja switches from an inline command line to a response file, so a small executable in the same project can link fine while a big one (e.g. the GUI target) fails -- do not read "some targets link, one doesn't" as evidence the smaller ones are more correct; check whether the failing one is the one using an `@rsp`. Fix (not yet implemented anywhere): teach the wrapper script that receives the args to expand `@file` itself using Windows-quoting rules before handing individual arguments to the GCC-driver binary, rather than passing `@file` straight through for clang to reparse. (alien, windows-gfx1151: hit on the `alien.exe` GUI target only; the compute-path executables (EngineTests, PersisterTests, cli, EngineInterfaceTests) stayed under the threshold and linked cleanly, so this did not block GPU validation.)
