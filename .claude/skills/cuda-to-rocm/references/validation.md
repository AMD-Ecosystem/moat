...
A fresh `VCPKG_ROOT` clone must NOT be shallow (`--depth 1`). A shallow vcpkg clone fails the first manifest configure with `vcpkg install failed` / "vcpkg was cloned as a shallow repository... failed to git show versions/baseline.json" for every port in the manifest -- versioned-baseline resolution walks vcpkg's own commit history, not just the tip tree. This looks like a project-build failure but is purely a vcpkg-checkout mistake: reclone in full (no `--depth`) and `./bootstrap-vcpkg.sh` again; no project source or CMake change is involved. (CubbyFlow, second validating arch after the first arch's clone happened to already be a full clone)
