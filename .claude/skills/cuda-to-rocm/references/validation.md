...
- PR-prep gate -- upstream drift: `git fetch` the fork's default branch and check `git rev-list --count <port-branch>..origin/<default>` is 0 before review, and again before opening upstream. **`git diff <default>...<branch>` will not tell you** -- three dots diffs from the MERGE BASE, so a branch cut months ago shows a clean, plausible diff against code upstream has since rewritten. GooFit sat 19 commits behind through two full review rounds this way; one of those commits repointed the very submodule the port special-cased and rewrote the exact CMake block the port wrapped, and the diff under review was not the diff that would have been proposed. Rebase and re-run the suite, do not merge: a merge leaves the upstream PR carrying somebody else's commits. Two things to look for after rebasing: whether upstream has independently adopted part of your change (drop your copy rather than proposing it twice), and whether a project-wide probe now answers differently (GooFit's `check_ipo_supported()` flipped NO -> YES across the rebase, so every conclusion anyone had drawn from the cached value was stale). Re-measure such a probe rather than trusting a cache or an earlier review; and note that what a probe reports and what the build generates are different questions, so settle a flag argument by reading the generated `build.ninja`. (GooFit)
## Platforms
