...
- `torch.utils.cmake_prefix_path` can lie when the host's torch is an editable/source dev checkout, not a wheel. It is computed from `os.path.dirname(torch.__file__)/share/cmake`; an editable (scikit-build-core-style) install can repoint `import torch` at a raw source tree whose own `build/` directory is a bare ninja output, not a `cmake --install` tree -- its `TorchConfig.cmake`/`Caffe2Targets.cmake` derive their install prefix by walking a fixed number of parent directories from the `.cmake` file's own location, correct only for the exact nested path CMake originally exported it to, so pointing `CMAKE_PREFIX_PATH` straight at that `build/` surfaces as `find_package(Torch)` handing back a nonexistent `INTERFACE_INCLUDE_DIRECTORIES` (e.g. `/var/lib/include`) rather than a clean failure. Fix by finding the OTHER torch: check whether the same Python environment's `site-packages/` also holds a complete, separate `torch/` payload (real `share/cmake/Torch/TorchConfig.cmake`, real `include/`, real `lib/*.so`) alongside the editable shim (`_editable_*.pth`/`.py` markers) -- a from-wheel install and an editable dev pointer can coexist, the editable shim only redirects the *Python* `import`, and the complete sibling directory is usually what a downstream C++ extension should build against: `-DCMAKE_PREFIX_PATH=<env>/site-packages/torch/share/cmake` directly, bypassing `torch.utils.cmake_prefix_path` entirely. Two things that look like fixes but are not: copying `Caffe2Targets.cmake` up one directory computes a DIFFERENT wrong prefix, because the parent-walk count is baked in per export depth; and setting `TORCH_INSTALL_PREFIX` in the environment fixes `TorchConfig.cmake`'s own prefix (it explicitly honors that variable) but not `Caffe2Targets.cmake`'s independently-derived one, so `Found Torch` succeeds and the include-path error just moves one step later, at generate time. (Quest, linux-gfx1100 revalidation)
