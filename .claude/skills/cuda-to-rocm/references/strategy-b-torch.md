...
- The same trap fires on ANY pip install that has torch in its dependency graph, not just a requirements file, and a source-built ROCm torch is the worst case. `pip install timm torch_geometric` on a host whose torch is a locally built dev wheel (`2.14.0a0+git<sha>`) replaced torch with the PyPI CUDA build, pulled the whole `nvidia-*` stack, and swapped `triton` 3.8.0 for the CUDA `triton` 3.7.1 -- the dev version is on no index, so pip cannot put it back and nothing warns you. Default to `pip install --no-deps <pkg>` for every dependency on such a host, add the handful of pure-Python transitive deps by hand, and before you start, find the wheel that produced the current torch (`/var/lib/jenkins/pytorch/dist/*.whl` on a source-built host, plus the matching triton wheel) so a restore is `pip install --no-deps --force-reinstall <wheel>`. Verify with `python -c "import torch; print(torch.__version__, torch.version.hip)"` after any install that touched a torch-dependent package. (Pointcept, 2026.)
...
- A common dep usually already has a ROCm port -- consume it, do not re-port it. Apply the existing-AMD-support test (above) to each DEPENDENCY, not just the top repo: `spconv` -> `spconv-triton` (Apache-2.0 on PyPI, a drop-in for the spconv 2.x API in Triton; `spconv_rocm` is a name that looks like a port and is an empty repo -- checked 2026), `torch_scatter`/`torch_cluster` -> the pyg-rocm-build wheels or a source build (both build on ROCm 7.14 with `PYTORCH_ROCM_ARCH=<arch> FORCE_ONLY_CUDA=1 pip install . --no-build-isolation --no-deps`; torch_scatter 2.1.2 needs its `at::Half` shuffle wrappers widened to a 64-bit mask first, see the Pointcept notes), `gsplat` -> AMD's `amd_gsplat`. (So a dependency being un-portable at the source level does not by itself block a downstream consumer -- a dep-level ROCm substitute may exist.) Consume these via the `_deps/` mechanism; they are environment, not deliverables.
