...
A maintainer may decline the merge but adopt the fork by reference: close the PR and point their users at `moat-port` (aihwkit's README does). A person records that ruling with `moatlib.py set-maintained`; the branch is then a long-lived public deliverable -- no follow-up upstream PR is ever offered, the pre-push freeze stays on `moat-port` exactly as under an open PR, and `upstream.py --drift` watches upstream for conflicts with the port (the moat-checkup skill runs it). Upstream's advance is absorbed through sync rounds: `moatlib.py sync-branch` stages `moat-sync-<sha>`, which merges -- never rebases -- the upstream tip and runs the same porter/reviewer/validator cycle and gates as a fix round. The approved round merges through `upstream.py --merge-fix --apply`, which also tags the tip (`rocm-<date>`) so followers have fixed points; `--drift --apply` keeps the fork's default-branch mirror fast-forwarded.
