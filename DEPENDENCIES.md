...
- A dependency clears four different ways, and they are NOT the same answer:
  ...
  - **unknown** -- nobody has looked at it. File an intake request with `python3 utils/port_request.py file <owner/repo> --blocks <project> --why "..." --apply`, record the edge with `set-deps` (which REPLACES the whole `depends_on` list -- repeat the existing deps alongside the new one), and stop. A person decides whether to fork it.
...
If a project is (or is likely to be) a dependency of another MOAT target, its `notes.md` MUST include a `## Install as a dependency` section giving: the exact configure + build + install commands (with the HIP flags and arch-from-`CMAKE_HIP_ARCHITECTURES`), the install-prefix layout, and what a dependent sets to consume it (the `find_package` package name and/or the include + link flags). A base library that several targets consume especially needs it. Checked, not remembered: `python3 utils/moatlib.py dep-doc-gaps` lists every provider a recorded `depends_on` names whose notes.md lacks the section, and `orient.sh` prints the gaps as a standing nag. Write the section from the provider's own validated build (its notes.md recipes), on the provider's branch -- never guess one.

## Known dependency graphs

Module-level (not hard `depends_on`):
- `cupoch`'s deferred `imageproc` module needs `libSGM` (CUDA semi-global-matching stereo). `libSGM` has since been ported as its own MOAT project (review-passed, upstream PR open), so the module is buildable whenever someone picks it back up; the cupoch core does not need it.
- `RXMesh`'s matrix/solver/diff module needs the low-level cusolverSp csrqr API (ROCm/hipSOLVER#443, filed) and a cuDSS-class GPU direct solver (maps to STRUMPACK, dispositioned `already-supported` -- upstream STRUMPACK already runs on HIP/ROCm, so it needs no port from us). The module is header-only; the delivered RXMesh core + mesh-query + dynamic-editing port does not need it. RXMesh's status.json nonetheless records `depends_on: ["STRUMPACK"]` -- the edge reads `ok` through the disposition, so it gates nothing, but by the rule above a module-level dependency belongs in notes.md; drop the edge next time that record is edited on its branch.
